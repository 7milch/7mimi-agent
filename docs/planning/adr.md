...
Decision: `ai-it-x-daily-digest` の X シグナル収集を、orchestrator の事前収集(signals.json 投入)から、runner 内 Claude Code が auth-proxy の `/mcp` を Streamable HTTP MCP(`--mcp-config`、`Authorization: Bearer <session token>`)で直接叩く方式へ移行する(env `X_MCP_DIRECT=1` で opt-in、既定は従来経路)。認可はセッション token に role を紐付けて行う: orchestrator が `POST /session/issue`(静的 AUTH_PROXY_SESSION_TOKEN で認証)で role 付き短命 token(TTL 約35分)を発行し、`/mcp` が Go 側で role×tool を決定的に判定(tools/list も role で filter、許可外は JSON-RPC error + block 監査)。同一 token を `/mcp`(role 解決)と `/git`(有効性のみ)で共用**できる**(git relay は minted token を受理する)が、現行の runner 実装は git には従来の静的 `GIT_PROXY_SESSION_TOKEN` を渡し、minted token は `/mcp` のみで用いる。`/v1/slack/notify` は orchestrator 側のため静的 token を維持する。コスト制御は prompt guardrail(検索上限・max_results≤10・再試行禁止)を補助とし、決定的なバックストップとして `/mcp` にセッション単位のハード呼び出し上限(`AUTH_PROXY_MCP_CALL_CAP`)を置く。`/mcp` は read-only な evidence/signal tool のみを載せ、publish 系(git relay / Slack)は自己選択サーフェスに絶対に載せない(不変条件)。Claude Code のバージョンは Dockerfile で pin する(HTTP-MCP ヘッダ付与の回帰を防ぐため)。invest digest は本移行のスコープ外とし、ai-it の実運用検証(ハードキャップが上限を実際に効かせた/注入起因のツール暴走なし)後に別途移行する。
